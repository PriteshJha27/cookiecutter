...
# Step 1: Initialize MiniLM and LLM
...
def build_knowledge_graph(triplets):
    """
    Build a Knowledge Graph from triplets.
    """
    graph = nx.DiGraph()
...
triplets = [
    ...
    ("Data Analysis", "involves", "Extracting Insights"),
]
...
def decompose_query(query):
    """
    Decompose a complex query into simpler logical steps.
    """
    # Example: (X, "used_for", ?) => Projection on "used_for"
    projections = []
    if "used_for" in query:
        projections.append((query.split()[0], "used_for"))
    return projections

query = "What is Python used for?"
decomposed_query = decompose_query(query)

# Step 4: Neighborhood Retrieval (DFS for k-hop neighbors)
def retrieve_neighborhood(graph, start_node, k=2):
    """
    Perform k-hop Depth-First Search to retrieve a contextual subgraph.
    """
    neighborhood = nx.ego_graph(graph, start_node, radius=k, undirected=True)
    return neighborhood
...
# Step 5: Format LLM Prompt
def format_prompt(entity, neighborhood, query):
    """
    Convert the neighborhood subgraph and query into a structured prompt for LLM.
    """
...
prompt = format_prompt(start_entity, neighborhood, query)

# Step 6: LLM Reasoning
def perform_reasoning(prompt):
    """
    Use ChatOpenAI to process the logical reasoning query.
    """
